...
int main_learn()
{
	...
	learn_argv[0] = learn_Param_Names[0] ;

	learn_itr_cnt = 0;

	for(learn_itr_cnt=0;learn_itr_cnt<MAX_MODEL_CNT;learn_itr_cnt++)
	{
		learn_argv[1] = learn_Param_Names[(learn_itr_cnt*2)+1] ;
		learn_argv[2] = learn_Param_Names[(learn_itr_cnt*2)+2] ;

		puts("\n---------------\n")  ;
		printf("learn_argc = %d\nargv[0] = %s\nargv[1] = %s\nargv[2] = %s\n", learn_argc, learn_argv[0], learn_argv[1], learn_argv[2]) ;

		learn_ret = SVM_learn(learn_argc, learn_argv, learn_itr_cnt );
	}
...
int SVM_learn(int argc, char* argv[], int itr)
{  
  ...
  read_input_parameters_learn(argc,argv,learn_docfile,learn_modelfile,restartfile,&verbosity,
			&learn_parm,&kernel_parm);
  read_documents(learn_docfile,&docs,&target,&totwords,&totdoc);
  ...
  write_model(learn_modelfile,model);
...
void read_input_parameters_learn(int argc,char *argv[],char *learn_docfile,char *learn_modelfile,
			   ...
			   LEARN_PARM *learn_parm,KERNEL_PARM *kernel_parm)
{
  ...
  /* set default */
  strcpy (learn_modelfile, "svm_model");
  ...
  strcpy (learn_docfile, argv[i]);
  if((i+1)<argc) {
    strcpy (learn_modelfile, argv[i+1]);
  }
